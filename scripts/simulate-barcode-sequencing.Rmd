...
```{r,generate-test-dataset,message=F,cache=T}
set.seed(123)
fakedatar_list <- list()
...
```{r,prelim-checks-on-fake-data,cache=T}
...
# neighbors
identifyByHamming <- function(this_barcode,mismatch_threshold=1) {
    #
...
buildAdjacency <- function(supplied_barcodes) {
    this_df <- Reduce(function(x,y) merge(x,y,all=T),
        lapply(supplied_barcodes,identifyByHamming)
        )
    this_df <- merge(
        this_df,
        Reduce(function(x,y) merge(x,y,all=T),
            lapply(names(this_df[,-1]),identifyByHamming)
            ),
        all=T
        )
...
this_fake_datar <- filter(fakedatar,number_barcodes==10,
    barcode_frequencies=="pois",barcode_length==4,
    sequencing_depth==1e4,error_rate_substitution==.005)
